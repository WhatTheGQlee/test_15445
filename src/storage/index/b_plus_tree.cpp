...
auto BPLUSTREE_TYPE::GetValue(const KeyType &key, std::vector<ValueType> *result, Transaction *transaction) -> bool {
  ...
  bool found = false;
  Page *page = GetLeafPage(key, transaction, OpeType::FIND);
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::GetLeafPage(const KeyType &key, Transaction *transaction, OpeType op) -> Page * {
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value, Transaction *transaction) -> bool {
  ...
  if (IsEmpty()) {
    ...
    bool ret = leaf_page->Insert(key, value, comparator_);
    buffer_pool_manager_->UnpinPage(root_page_id_, true);
    root_latch_.WUnlock();
    return ret;
  }

  Page *page = GetLeafPage(key, transaction, OpeType::INSERT);
  LeafPage *leaf_page = CastLeafPage(page);
  // for (int i = 0; i < leaf_page->GetSize(); ++i) {
  ...
  InsertInParent(leaf_page, new_leaf_page, key0);
...
void BPLUSTREE_TYPE::InsertInParent(BPlusTreePage *left_node, BPlusTreePage *right_node, const KeyType &key0,
  ...
  Page *parent_page = buffer_pool_manager_->FetchPage(left_node->GetParentPageId());  // pin2
  ...
  if (parent_node->GetSize() < parent_node->GetMaxSize()) {
    ...
    InsertInParent(parent_node, right_parent_node, parent_key0);

    buffer_pool_manager_->UnpinPage(right_parent_page_id, true);  // unpin3
  }
  buffer_pool_manager_->UnpinPage(parent_node->GetPageId(), true);  // unpin2
}
...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *transaction) {
  ...
  root_latch_.WLock();
  Page *page = GetLeafPage(key, transaction, OpeType::REMOVE);
  if (page == nullptr) {
    return;
  }
...
auto BPLUSTREE_TYPE::Begin(const KeyType &key) -> INDEXITERATOR_TYPE {
  // std::scoped_lock<std::mutex> lock(latch_);
  root_latch_.RLock();
  Page *page = GetLeafPage(key, nullptr, OpeType::FIND);
